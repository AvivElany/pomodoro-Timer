...
function startTimer() {
  ...
  const endTime = Date.parse(new Date()) + total * 1000;
    ...
    if (total <= 0) {
        ...
        const audioElement = document.querySelector(`[data-sound="${timer.mode}"]`);
        if (audioElement) {
            audioElement.play().catch(e => {
                console.warn('Could not play timer sound:', e);
            });
        }
        stopTimer(); // עצירת הטיימר במקום הפעלה מחדש
    }
    }, 1000);
}
